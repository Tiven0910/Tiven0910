<!-- ============================================================= -->
<!--                     TIVENTHAN — GITHUB PROFILE               -->
<!-- ============================================================= -->

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,24&height=220&section=header&text=Tiventhan&fontSize=72&fontColor=ffffff&animation=fadeIn&fontAlignY=34&desc=Full-Stack%20Developer%20%C2%B7%20Laravel%20%C2%B7%20PHP%20%C2%B7%20Kotlin%20Multiplatform&descSize=18&descAlignY=54" alt="header" />

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1000&color=8B5CF6&center=true&vCenter=true&width=680&lines=Full-Stack+Developer;Laravel+%7C+PHP+%7C+PostgreSQL+%7C+REST+APIs;Kotlin+Multiplatform+Mobile;Final-year+Bachelor+of+Computer+Science+(Hons)" alt="Typing SVG" />
</a>

<br/>

![Degree](https://img.shields.io/badge/BCS%20(Hons)-CGPA%203.67%2F4.00-7C3AED?style=for-the-badge&logo=googlescholar&logoColor=white)
![University](https://img.shields.io/badge/Universiti%20Selangor-4F46E5?style=for-the-badge&logo=academia&logoColor=white)
![Location](https://img.shields.io/badge/Malaysia-6D28D9?style=for-the-badge&logo=googlemaps&logoColor=white)

<br/>

<a href="https://tiven-portfolio.vercel.app">
  <img src="https://img.shields.io/badge/Portfolio-6D28D9?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
</a>
<a href="https://www.linkedin.com/in/tiventhan-63079534b">
  <img src="https://img.shields.io/badge/LinkedIn-4F46E5?style=for-the-badge&logo=data:image/svg%2Bxml;base64,&logoColor=white" alt="LinkedIn" />
</a>
<a href="mailto:tiventhan123@gmail.com">
  <img src="https://img.shields.io/badge/Email-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
<a href="https://github.com/Tiven0910">
  <img src="https://img.shields.io/badge/GitHub-1a1a2e?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=Tiven0910&style=for-the-badge&color=6D28D9&label=PROFILE+VIEWS" alt="views" />
<img src="https://img.shields.io/github/followers/Tiven0910?style=for-the-badge&color=6D28D9&labelColor=1a1a2e&logo=github" alt="followers" />
<img src="https://img.shields.io/github/stars/Tiven0910?style=for-the-badge&color=6D28D9&labelColor=1a1a2e&logo=github" alt="stars" />

</div>

---

## 👨‍💻 About Me

Final-year **Bachelor of Computer Science (Honours)** student and **Full-Stack Developer** who builds production web applications end to end.

- Backend-strong: **Laravel / PHP 8**, REST APIs, PostgreSQL & MySQL, role-based workflows.
- Full-stack: JavaScript, Tailwind, Bootstrap & Blade — schema to polished UI.
- Shipped production features used by major Malaysian enterprises — **Malaysia Airports (MAHB), Pos Malaysia, and CTOS**.
- Hands-on **Kotlin Multiplatform** mobile experience (Jetpack Compose).
- ~2 years delivering custom software for real freelance clients, end to end.

**Open to:** `Full-Stack Developer` · `Web Developer` roles — Malaysia / Remote.

---

## 🧰 Tech Stack

<div align="center">

**Languages**

![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css&logoColor=white)

**Frontend**

![Tailwind](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat-square&logo=bootstrap&logoColor=white)
![Blade](https://img.shields.io/badge/Blade-F05340?style=flat-square&logo=laravel&logoColor=white)

**Backend & Databases**

![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black)
![REST API](https://img.shields.io/badge/REST_API-6D28D9?style=flat-square&logo=fastapi&logoColor=white)

**Cloud, DevOps & Tooling**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)

<br/>

<img src="https://skillicons.dev/icons?i=php,laravel,kotlin,java,js,html,css,tailwind,bootstrap,spring,postgres,mysql,redis,firebase,git,github,docker,figma,vscode,androidstudio&theme=dark" alt="skill icons" />

</div>

---

## 🗂️ Web Development Expertise

<div align="center">

| Domain | Proficiency | Details |
| :--- | :---: | :--- |
| **Backend Development** | `●●●●●` | Laravel · PHP 8 · Eloquent · REST APIs · role-based workflows |
| **Databases** | `●●●●○` | PostgreSQL · MySQL · schema design · query optimization |
| **Frontend Development** | `●●●●○` | JavaScript (ES6+) · Tailwind · Bootstrap · Blade · responsive UI |
| **Mobile Development** | `●●●○○` | Kotlin Multiplatform · Jetpack Compose |
| **DevOps & Delivery** | `●●●○○` | Git/Gitea · CI/CD (Gitea Actions) · Docker · SIT → UAT → Prod |

</div>

---

## 🚀 Featured Projects

<details open>
<summary><b>&nbsp;KollectValley — Enterprise Accounts-Receivable Platform</b></summary>
<br/>

Full-stack development on a multi-tenant Laravel platform serving major Malaysian enterprises, contributing complete production modules end to end.

| Aspect | Detail |
| :--- | :--- |
| **Stack** | Laravel 12 · PHP 8 · PostgreSQL · Bootstrap · JavaScript |
| **Scale** | Multi-tenant · 3 enterprise clients (MAHB · Pos Malaysia · CTOS) |
| **Performance** | Server-side DataTables · batch PDF/ZIP generation · scheduled jobs |
| **Security** | Role-based access control (6 roles) · session auth |
| **Impact** | Shipped Dispute Manager module, DCA analytics dashboard & automated workflows to production |
| **Repository** | `Private — company codebase` |

</details>

<details>
<summary><b>&nbsp;KollectMobile-Core — Kotlin Multiplatform Field App</b></summary>
<br/>

Cross-platform Android field-collections app on feature-first Clean Architecture, backed by a Spring Boot proxy service.

| Aspect | Detail |
| :--- | :--- |
| **Stack** | Kotlin Multiplatform · Jetpack Compose · Ktor · Spring Boot (backend) |
| **Scale** | Single APK · feature-first Clean Architecture |
| **Performance** | Offline-first drafts · background sync |
| **Security** | Bearer/JWT auth via Central proxy |
| **Impact** | Built Compose UI (Cases Tab, Borrower Profile) from Figma via MCP design-to-code |
| **Repository** | `Private — company codebase` |

</details>

<details>
<summary><b>&nbsp;Dance Academy Management System</b></summary>
<br/>

Full PHP/MySQL platform delivered for a freelance client — a responsive marketing site plus a secure self-service admin dashboard.

| Aspect | Detail |
| :--- | :--- |
| **Stack** | PHP · MySQL · JavaScript (Fetch/AJAX) · Tailwind CSS |
| **Scale** | 5-page site + admin dashboard · 3 self-service modules |
| **Performance** | AJAX-driven UI · JSON API endpoints |
| **Security** | Session auth · bcrypt hashing · prepared statements |
| **Impact** | Replaced manual workflows with a client-managed CMS |
| **Repository** | `Private — client project` |

</details>

<details>
<summary><b>&nbsp;Smart Job Seeking System — Final Year Project</b></summary>
<br/>

A job portal connecting seekers and companies, with full system architecture, database design, and multi-role dashboards.

| Aspect | Detail |
| :--- | :--- |
| **Stack** | HTML · CSS · JavaScript · PHP · MySQL · Firebase |
| **Scale** | Multi-role (seeker / company) · dashboards |
| **Performance** | Search & application flows |
| **Security** | Authentication & role-based access |
| **Impact** | End-to-end architecture, DB design & UI |
| **Repository** | [github.com/Tiven0910](https://github.com/Tiven0910) |

</details>

---

## 💼 Experience

**Web Developer Intern** — *Kollect Systems Sdn Bhd*
`Jan 2026 – Present · Malaysia`

Full-stack web development on **KollectValley**, an enterprise accounts-receivable platform delivered to Malaysia Airports (MAHB), Pos Malaysia, and CTOS.

- Built a complete role-based **Dispute Manager** module with access control across six user roles.
- Designed an internal analytics dashboard — interactive charts, KPI metrics, and status filtering.
- Automated overdue-dispute handling with a scheduled reminder-and-auto-release system.
- Built a bulk invoice-email feature (Pos Malaysia): batch PDF/ZIP generation with server-side processing.
- Contributed Jetpack Compose UI to KollectMobile-Core, a Kotlin Multiplatform Android app.

`Laravel` `PHP` `PostgreSQL` `Bootstrap` `JavaScript` `Kotlin Multiplatform` `Jetpack Compose`

<br/>

**Independent Web Developer** — *Freelance*
`2023 – Present · Remote / Malaysia`

- Delivered custom PHP/MySQL web systems for business clients, requirements → deployment → maintenance.
- Replaced manual workflows with database-driven CRUD modules and secure admin dashboards.
- Managed multi-environment deployment with Git branch promotion and Gitea webhook auto-deploy.

`PHP` `MySQL` `JavaScript` `Tailwind CSS`

---

## 🏆 Highlights

<div align="center">

| Highlight | Details |
| :--- | :--- |
| **Enterprise Delivery** | Shipped production features to MAHB, Pos Malaysia & CTOS |
| **Module Ownership** | Built a complete role-based Dispute Manager module from scratch |
| **Analytics** | Designed an internal DCA dashboard (charts, KPIs, filtering) |
| **Academic** | BCS (Hons) — CGPA 3.67 / 4.00 |
| **Freelance** | ~2 years delivering client software end to end |

</div>

---

## 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Tiven0910&hide_border=true&background=0d1117&stroke=8B5CF6&ring=8B5CF6&fire=A78BFA&currStreakLabel=8B5CF6&sideNums=c9d1d9&currStreakNum=c9d1d9&dates=8b949e&sideLabels=c9d1d9" alt="streak" />

</div>

<!--
github-readme-stats (stats card + top-langs) and github-profile-trophy are
currently returning HTTP 503 / 402 — their free public instances are over
quota. Uncomment once those services are healthy again.

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Tiven0910&show_icons=true&count_private=true&hide_border=true&title_color=8B5CF6&icon_color=A78BFA&text_color=c9d1d9&bg_color=0d1117" alt="stats" />
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Tiven0910&layout=compact&hide_border=true&langs_count=8&title_color=8B5CF6&text_color=c9d1d9&bg_color=0d1117" alt="top langs" />
<img src="https://github-profile-trophy.vercel.app/?username=Tiven0910&theme=algolia&no-frame=true&no-bg=true&margin-w=6&column=7" alt="trophies" />
-->

---

## 🧊 3D Contribution Graph

<div align="center">

<img width="100%" src="./profile-3d-contrib/profile-season-animate.svg" alt="3D contribution graph" />

</div>

---

## 📈 Contribution Activity

<div align="center">

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Tiven0910&bg_color=0d1117&color=8B5CF6&line=8B5CF6&point=A78BFA&area=true&hide_border=true" alt="activity graph" />

</div>

---

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Tiven0910/Tiven0910/output/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Tiven0910/Tiven0910/output/github-contribution-grid-snake.svg">
  <img width="100%" alt="github contribution snake" src="https://raw.githubusercontent.com/Tiven0910/Tiven0910/output/github-contribution-grid-snake-dark.svg">
</picture>

</div>

---

## 🎯 Current Focus

```yaml
Learning:   [Advanced Laravel, System Design, Cloud Deployment]
Building:   [Production full-stack web apps, this developer portfolio]
Exploring:  [Kotlin Multiplatform, Spring Boot, Clean Architecture]
Open_To:    [Full-Stack / Web Developer roles — Malaysia / Remote]
```

---

## 🤝 Connect

<div align="center">

<a href="mailto:tiventhan123@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
</a>
<a href="https://www.linkedin.com/in/tiventhan-63079534b">
  <img src="https://img.shields.io/badge/LinkedIn-4F46E5?style=for-the-badge&logo=data:image/svg%2Bxml;base64,&logoColor=white" alt="LinkedIn" />
</a>
<a href="https://github.com/Tiven0910">
  <img src="https://img.shields.io/badge/GitHub-1a1a2e?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>
<a href="https://tiven-portfolio.vercel.app">
  <img src="https://img.shields.io/badge/Portfolio-6D28D9?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
</a>

</div>

---

<div align="center">

<i>"Building software that ships and holds up in production."</i>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,24&height=120&section=footer" alt="footer" />

</div>
